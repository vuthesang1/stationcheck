...
public class MockFaceRecognitionService : IFaceRecognitionService
{
    ...
    public async Task<FaceRecognitionResult> RecognizeFaceAsync(byte[] imageData)
    {
        ...
        // Mock: Tạo face location
        var faceLocation = new FaceLocation
        {
        ...
        var confidence = (float)(_random.NextDouble() * 0.25 + 0.7); // 0.7-0.95

        return new FaceRecognitionResult
        {
            FaceDetected = true,
            FaceRecognized = confidence >= _confidenceThreshold,
            EmployeeId = recognizedEmployeeId,
            EmployeeName = $"Employee {recognizedEmployeeId}",
