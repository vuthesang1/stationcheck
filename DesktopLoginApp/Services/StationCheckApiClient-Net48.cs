...
namespace DesktopLoginApp.Services
{
    ...
    /// .NET Framework 4.8 compatible version
    /// </summary>
    public class StationCheckApiClient : IDisposable
    {
        ...
        public async Task<DesktopLoginTokenResponse> RequestDesktopLoginTokenAsync(string jwtToken)
        {
            ...
            try
            {
                ...
                {
                    var result = JsonConvert.DeserializeObject<DesktopLoginTokenResponse>(rawContent);
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    try
                    {
                        ...
                        var result = JsonConvert.DeserializeObject<LoginResponse>(rawContent);
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        var errorResponse = JsonConvert.DeserializeObject<LoginResponse>(rawContent);
        ...
        public async Task<DeviceRegistrationResponse> RegisterDeviceAsync(string username, string password, string macAddress, string deviceName)
        {
            try
            {
                ...
                var jsonContent = JsonConvert.SerializeObject(request);
                var content = new StringContent(jsonContent, Encoding.UTF8, "application/json");
                var response = await _httpClient.PostAsync("/api/auth/register-device", content);

                if (response.IsSuccessStatusCode)
                {
                    var rawContent = await response.Content.ReadAsStringAsync();
                    var result = JsonConvert.DeserializeObject<DeviceRegistrationResponse>(rawContent);
        ...
        public async Task<DeviceStatusResponse> GetDeviceStatusAsync(Guid deviceId)
        {
            try
            {
                ...
                {
                    var rawContent = await response.Content.ReadAsStringAsync();
                    var result = JsonConvert.DeserializeObject<DeviceStatusResponse>(rawContent);
        ...
        private X509Certificate2 LoadClientCertificate()
        {
            try
            {
                using (var store = new X509Store(StoreName.My, StoreLocation.CurrentUser))
                {
                    ...
                    var certs = store.Certificates.Find(X509FindType.FindBySubjectName, "StationCheck", false);
